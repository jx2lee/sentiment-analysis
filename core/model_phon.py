...
def Build_stackedLSTMgraph(
    ...
    # Embedding-layer
    embeddings = tf.get_variable('embedding_matrix', [phon_size, state_size], trainable=False)
...
def Build_stackedRNNgraph(
    ...
    # RNN
    rnns = [tf.contrib.rnn.BasicRNNCell(size) for size in rnn_sizes]
    drops = [tf.contrib.rnn.DropoutWrapper(rnn, output_keep_prob=keepProb_) for rnn in rnns]
    cell = tf.contrib.rnn.MultiRNNCell(drops, state_is_tuple=True)
...
def Build_stackedGRUgraph(
    ...
    # Embedding-layer
    embeddings = tf.get_variable('embedding_matrix', [phon_size, state_size], trainable=False)
    gru_inputs = tf.nn.embedding_lookup(embeddings, x)

    # GRU-layer
    grus = [tf.contrib.rnn.GRUCell(size) for size in gru_sizes]
    drops = [tf.contrib.rnn.DropoutWrapper(gru, output_keep_prob=keepProb_) for gru in grus]
    cell = tf.contrib.rnn.MultiRNNCell(drops, state_is_tuple=True)

    init_state = cell.zero_state(batch_size, tf.float32)

    gru_outputs, final_state = tf.nn.dynamic_rnn(cell, gru_inputs, sequence_length=seqlen,
                                                 initial_state=init_state)
    
    # Extract last output
    idx = tf.range(batch_size)*tf.shape(gru_outputs)[1] + (seqlen - 1)
    last_gru_output = tf.gather(tf.reshape(gru_outputs, [-1, state_size]), idx)
    ...
    logits = tf.matmul(last_gru_output, W) + b
    preds = tf.nn.softmax(logits)
    correct = tf.equal(tf.cast(tf.argmax(preds,1),tf.int32), y)
    accuracy = tf.reduce_mean(tf.cast(correct, tf.float32))
...
def Train_graph_phon(graph, tr, te, batch_size, num_epochs, iterator, checkpoint_path, pkl_name, learn_time):
